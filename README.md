...
**Domain:** `Security / DevSecOps` · **Suite:** `Security Suite` · **Accent:** `#f87171`
...
**Secrets Scanner** is a standalone, browser-based AI workflow app for **Security / DevSecOps**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `rulesets`<br>rulesets | textarea | No | — | Affects customization: rulesets. |
| `allowlist`<br>allowlist | textarea | No | — | Affects customization: allowlist. |
| `severity_policy`<br>severity policy | text | No | — | Affects customization: severity policy. |
| `branches`<br>branches | text | No | — | Affects customization: branches. |
| `notification_channels`<br>notification channels | textarea | No | — | Affects customization: notification channels. |
| `remediation_templates`<br>remediation templates | text | No | — | Affects customization: remediation templates. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9151
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9151`. |
...
- Repo/code
- secret findings and remediation
...
```bash
curl http://127.0.0.1:9151/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9151/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9151 uvicorn app.main:app --host 0.0.0.0 --port 9151
```
...
- `plugins/secrets-scanner.json` for plugin metadata.
